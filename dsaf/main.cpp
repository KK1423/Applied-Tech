...
sf::RenderWindow window(sf::VideoMode(640, 480), "SFML works!");
	...
	for (unsigned y = 0; y < height; ++y) {
		for (unsigned x = 0; x < width; ++x, ++pixel) {
			...
			raydir.normalize();
			*pixel = trace(Vec3<T>(0,n,0), raydir, spheres, 0);
		}
	}
	n++;
	...
	for (unsigned i = 0; i < width * height; ++i) {
		shape.setPixel(i%640,(i-(i%640))/640,sf::Color((uint8_t)(std::min(T(1), image[i].x) * 255),(uint8_t)(std::min(T(1), image[i].y) * 255),(uint8_t)(std::min(T(1), image[i].z) * 255)));
